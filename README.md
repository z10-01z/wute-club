...
队标文件 `picture/WUTE_logo_white.png` 实际为蓝色原图；导航与页脚直接显示原色，不要添加反白滤镜。全站不使用蓝白红三色装饰短条。首页英文轮播大字使用 `造字工房力黑`，中文轮播大字使用 `狂派手书`；两款字体均使用 400 字重，不合成加粗。`MAXIMUM PUSH!` 保持单行；桌面首屏占满视口。其余标题继续使用共享字阶。
...
- **改共享 CSS / JS**：同时更新各 HTML 中 `wute.css?v=...` / `wute.js?v=...` 的版本号；若修改按需加载的 `wute-game.js`，也要更新 `assets/wute.js` 中对应的查询版本，让浏览器立即获取新文件。服务器可能缓存旧 URL 7 天
